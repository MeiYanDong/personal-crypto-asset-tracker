...
- 记忆点：把“总资产 -> 稳定资产 -> 波动资产 -> 保守折价”的关系直接可视化。
...
职责：在一个连续摘要带中展示总资产、保守估值、稳定资产/波动资产构成、折价缓冲、钱包/币种/链数量，并将常态链范围收纳到对应事实而非 Alert。
...
职责：按链汇总总资产、保守估值、稳定资产、钱包和主要持仓；继续使用账本表格承载精确值，移动端保留链、金额和钱包三个主判断。
...
### 2026-08-09 第一百八十八轮基线

观察与方法：

- 原登录门虽然由服务器校验 `ASSET_TRACKER_TOKEN`，但前端把口令长期写入 `localStorage`；同一台电脑再次打开页面会自动携带口令，无法满足“每次登录重新验证”的隐私目标。
- 钱包、资产组和快照可以继续使用浏览器缓存改善恢复体验，但认证口令不能与业务缓存共用持久化介质。两类状态必须分离：业务数据可恢复，认证凭据只存在于当前页面内存。
- 只在首次打开要求密码仍无法覆盖“页面保持打开、人离开电脑”的情况；本轮同时提供显式锁定和 5 分钟无操作自动锁定，不引入浏览器 Cookie 或新的服务端会话存储。
- 锁定是常用安全命令，桌面与移动端都使用始终可见的 Lucide `LockKeyhole` 图标按钮；移动端不把它藏进二级菜单。

本轮动作：

- API 请求改为读取页面内存中的 `runtimeAuthToken`；启动时主动删除旧 `asset-tracker-token`，不再向 `localStorage` 或 `sessionStorage` 写入口令。
- 页面刷新、新标签页和浏览器重开都会丢失内存口令并重新进入密码页；错误密码会清空内存令牌并保留可访问的错误提示。
- `/api/config` 返回 `authRequired`，只有服务器实际配置访问口令时才显示锁定按钮并启用 5 分钟无操作计时，未配置口令的本地开发环境不受影响。
- 手动锁定同步关闭设置、钱包导入、资产组管理和删除确认状态，避免重新登录后恢复锁定前的敏感弹层。
- 移动端顶栏调整为“刷新 / 更多 / 锁定”三轨；钱包管理页为“锁定 / 重新载入 / 添加钱包”，同步 Badge 在窄屏隐藏。

复核结果：

- 首次打开只渲染密码门；错误密码显示 `role=alert` 的“访问口令不正确”，正确密码进入资产页。
...
- 原界面只有浅色视觉，283 处固定颜色分散在摘要、账本、钱包管理、表单、弹层、状态与币种图标中；只反转页面背景会留下亮色组件并破坏链、资产组和风险状态的语义。
- 深色模式采用中性炭黑层级，保留品牌绿以及链、资产组、成功、警告、错误的既有色相；令牌层负责画布、表面、边界、文字、控件和状态，业务标识只调整深色环境中的明度与对比度。
- 初次访问跟随操作系统；用户手动切换后写入独立的 `asset-tracker-color-theme`，刷新和再次打开继续使用显式选择。主题状态不进入钱包、快照或服务端持久化协议。
- 主题控制是全局高频动作，登录门、资产总览和钱包管理都使用同一个带 Tooltip 的 Lucide 图标按钮；按钮的名称描述切换后的动作，并通过 `aria-pressed` 暴露当前状态。

本轮动作：

- 新增 `ColorTheme` 状态、初始化器和 `ThemeToggle` 原子组件；React 挂载前先写入根节点 `data-theme` 与 `color-scheme`，降低首屏主题闪烁。
- 建立浅色与深色两套语义令牌，并完整覆盖摘要、风险估值、刷新质量、四类账本、钱包管理、资产组、表单、菜单、Tooltip、Dialog、Toast、空状态和链/币种标识。
- Toast 跟随当前主题；登录门也提供主题切换，用户无需先暴露资产页面就能选择合适的显示模式。
- 680px 以下的钱包页动作栏保留 44px 主题与刷新图标按钮，隐藏刷新按钮的可见文字，修复固定轨道内的文字溢出；桌面仍显示完整命令。
- 新增 `check:theme`，覆盖显式选择优先、系统偏好回退和非法存储值回退，完整生产构建会执行该契约。

复核结果：

- 1440 x 900 的资产总览与钱包管理均没有高亮度残留表面，页面 `clientWidth / scrollWidth = 1440 / 1440`；主题切换后无需重新载入即可更新全部组件。
- 浅色选择刷新后仍保持浅色，切回深色后钱包管理继续为深色；主题按钮的 `aria-label` 和 `aria-pressed` 与状态同步。
- 390 x 844 钱包管理页为 `clientWidth / scrollWidth = 390 / 390`，主题、重新载入和添加钱包动作互不重叠，页面没有横向溢出。

### 2026-09-06 第一百九十一轮基线

观察与方法：

- 普通余额接口不能完整表达 Uniswap V3 NFT LP 等协议仓位；如果把 DeFi 底层资产伪装成普通 token，会丢失协议、池、区间与仓位身份，并容易重复计价。
- DeFi 总额和仓位明细的可靠性不同。协议总览成功而明细失败时，总额仍是可用资产事实，界面需要把“金额已更新”与“明细不完整”分开表达。
- DeFi 是资产观察维度，不是钱包管理能力。它与资产组、链、币种、钱包并列进入总览 tabs，钱包配置页保持专注。
- 保守估值暂不推断 LP 中稳定币比例，完整 DeFi 净值统一作为波动资产按 80% 计入，避免从不完整底层明细推导更精确但不可验证的分类。

本轮动作：

- 新增 DeFi 协议账本，桌面显示协议、金额、链分布、仓位、钱包、主要仓位和状态；移动端重排为协议账本条目，不依赖横向表格。
- 新增协议与仓位原子组件，远程 logo 失败时使用居中的确定性缩写图标；仓位支持 NFT token ID、价格区间、类型和底层代币。
- 链与钱包视图改为“币种 / DeFi”双计数，并在主要持仓中标明 DeFi 金额；搜索同时覆盖协议、仓位名称和底层代币。
- DeFi 明细低于 `$1` 时省略展示，协议净值仍完整计入总资产、链汇总、资产组汇总、钱包汇总和保守估值。
- 协议明细失败使用警告状态且不清空总额；总览失败时沿用旧 DeFi 快照。刷新设置可单独关闭 DeFi 扫描。

复核结果：

- 32 个地址组成的 16 个逻辑钱包完整刷新无错误；2 个钱包发现 DeFi。普通 token 总额与 DeFi 净值之和和全局及逐钱包总额完全一致。
- 本地 CLI 与使用生产环境变量的 OKX HTTP API 均成功读取相同的可见协议；协议总览、明细和 Vercel 运行路径均通过。
- 1440 x 900 与 390 x 844 的浅色、深色 DeFi 视图均无横向溢出；五个 tabs 高度均为 44px，协议 logo 的图像框在 40px 容器内居中，浏览器 warning/error 为 0。
- DeFi 解析契约、数值契约、主题契约、钱包配对、TypeScript 与 Vite 构建均通过；最终产物为 3 个 JS chunk、555.03 kB，gzip 168.63 kB。
- DeFi 账本使未压缩 JavaScript 基线从不超过 550 kB 增至不超过 560 kB；gzip 上限继续保持 175 kB，单 chunk 上限继续保持 500 kB。

### 2026-09-06 第一百九十二轮基线

观察与方法：

- 生产日志确认一次长时间资产刷新已返回 HTTP 200，但浏览器因到 Vercel 的 TLS 连接中断没有收到响应，界面直接暴露了 `Failed to fetch`，把服务端成功误报成操作失败。
- 普通 GET 和幂等 PUT 可以自动重试；资产刷新 POST 不应盲目重发，否则可能重复执行全钱包扫描。刷新请求需要可持久化的唯一身份，以服务器快照作为最终结果证明。

本轮动作：

- GET 与 PUT 遇到网络级失败时最多自动尝试三次并指数退避；连续失败统一显示中文网络状态，不再暴露浏览器原生错误。
- 每次资产刷新生成 `refreshRequestId` 并写入持久化快照。POST 响应丢失时读取云端快照，ID 一致则恢复为成功，不一致才提示结果尚无法确认。
- 响应体被截断导致 JSON 解析失败时同样进入网络恢复路径，覆盖已经收到响应头但正文不完整的情况。

### 2026-09-06 第一百九十三轮基线

观察与方法：

- “稳定币”不足以表达当前保守估值口径；用户确认 SPY 属于稳定资产，应与合格稳定币一起按 100% 计入。
- SPY 同时存在于普通余额和 Uniswap LP 底层资产。只修改现货符号列表会漏掉可验证的 LP 稳定部分；直接把全部 DeFi 视为稳定又会高估。

本轮动作：

- 估值分类拆为美元稳定币与命名稳定资产：稳定币继续要求隐含价格处于 `$0.90-$1.10`，SPY 不受该区间限制，两类都要求无风险标记。
- DeFi 明细中的 SPY 与合格稳定币按底层美元市值计入稳定资产，并以单仓位和协议总额双重封顶；借贷负债、无明细部分和其他底层资产继续按波动资产计入。
- 摘要、公式、资产组和链账本文案统一从“稳定币”升级为“稳定资产”；快照新增 `stableAssetUsd`，保留 `stablecoinUsd` 兼容旧数据与历史。

### 2026-09-07 第一百九十四轮基线

观察与方法：

- OKX DeFi 协议概览与逐仓位明细属于不同刷新层，概览金额可能滞后于同次返回的完整仓位明细。Uniswap V4 的概览总额曾高于 4 个明细仓位合计，导致总资产和保守估值一起偏高。
- Uniswap 的 `INACTIVE` 表示集中流动性处于价格区间外，不等于 NFT 已撤空；是否仍有资产必须结合 owner、liquidity 和仓位金额判断。
- LP 仓位 `totalValue` 可能高于底层代币明细之和，差额对应未领取费用或其他附加价值。Uniswap 页面可能将这部分分开展示，不能在资产口径中静默隐藏。

本轮动作：

- 明细数量覆盖协议报告仓位数时，协议和链总额统一改用逐仓位明细合计；明细不足时保留协议概览并在界面标记“概览估值”或“混合估值”。
- 仓位行将 `ACTIVE/INACTIVE` 翻译为“区间内/区间外”，并显示可识别的“费用/其他”金额。
- 旧快照在服务端和浏览器端读取时重新执行同一估值协调逻辑，避免只修新刷新数据。
