...
刷新范围默认开启 DeFi 扫描。系统先获取每个钱包的协议总览，再对价值不低于 `$1` 的协议网络读取仓位明细。Uniswap V3/V4 LP 会保存协议、网络、NFT token ID、价格区间和底层代币；其他协议按接口返回的仓位类型展示。

DeFi 净值单独保存在快照中，并只计入总资产一次。已识别为 LP 凭证的普通 token 余额会从普通持仓中排除，减少 V2 LP 重复计价。完整仓位明细可用时，协议总额由逐仓位金额合计，不采用可能滞后的协议概览金额；明细数量不足时才回退到概览估值。LP 仓位总额可能包含未领取费用，界面将底层资产与“费用/其他”分开标注。单个仓位或协议低于 `$1` 时页面省略明细，但完整金额仍保留在快照和资产估值中。
...
```text
保守资产估值 = 稳定资产市值 + (总资产市值 - 稳定资产市值) × 0.8
```

稳定资产部分保留 100%，其余波动资产按 80% 计入。稳定资产包括 SPY，以及 USDT、USDC、USDT0、USDG、DAI、USDS、FDUSD、PYUSD、USDP、TUSD、BUSD、GUSD 和常见 USDC/USDT 跨链版本。美元稳定币只有价格处于 `$0.90–$1.10` 且未被标记为风险 token 时才按稳定资产处理；SPY 不受一美元价格区间限制，但仍要求无风险标记。DeFi 仓位会把明细中可验证的 SPY 和稳定币底层市值计入稳定资产，并以仓位及协议总额为上限；明细缺失、借贷负债和其余 DeFi 部分继续按波动资产处理。该计算使用完整资产快照，包含页面省略显示的 `<$1` 小额持仓。
